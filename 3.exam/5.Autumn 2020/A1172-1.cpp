...
int main(int argc, char const *argv[]) {
    ...
    vector<int> left(n + 1), right(n + 1); // count of strict decreases in a non-increasing sequence
    for (int i = 1; i < n; i++) {
        for (int j = i; j < n && weight[j] >= weight[j + 1]; j++) {
    ...
    for (int i = n; i > 1; i--) {
        for (int j = i; j > 1 && weight[j] >= weight[j - 1]; j--) {
            if (weight[j] > weight[j - 1]) {
                left[i]++;
            }
        }
    ...
    int res = 0;
    for (int i = 1; i <= n; i++) {
        res += max(left[i], right[i]) * 100 + 200; // peak: additional + base
    }
    cout << res << "\n";

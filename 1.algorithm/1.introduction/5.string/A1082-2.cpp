...
int main(int argc, char const *argv[]) {
    ...
    bool leadingZero = false, part = false; // for each part or group of 4 digits
    for (int i = 0; i < (int)s.size(); i++) {
        ...
        if ((s.size() - 1 - i) % 4 == 0) { // new part or group
            // size - 1 - i: the number of digits after the current digit
            if (s[i] == '0' && part) res += weight[s.size() - i];
            leadingZero = false, part = false;
        }
    }
    if (res[0] == ' ') res.erase(0, 1); // positive
    cout << res << "\n";

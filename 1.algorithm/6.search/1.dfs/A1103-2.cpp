...
void dfs(int factor, int factorCnt, int factorSum, int factorPowerSum) {
    if (factorCnt == k && factorPowerSum == n) { // base case
    ...
    if (factor <= 0 || factorCnt > k || factorPowerSum > n) { // pruning
        return;
    }
    if (factorPowerSum + (k - factorCnt) * factorPower[factor] < n) { // pruning
        // even if choose the largest factor, still can't reach n
        return;
    }
    temp.emplace_back(factor);
    dfs(factor, factorCnt + 1, factorSum + factor, factorPowerSum + factorPower[factor]); // choose
    temp.pop_back();
    dfs(factor - 1, factorCnt, factorSum, factorPowerSum); // not choose
}

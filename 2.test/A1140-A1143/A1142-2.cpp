...
bool isConnected(vector<int>& seq, int size, int v) {
    // check whether vertex v is connected to all previous vertices: seq[1..size)
...
bool isClique(vector<int>& seq) {
    for (int i = 1; i < (int)seq.size(); i++) {
        int v = seq[i];
        if (!isConnected(seq, i, v)) { // all previous vertices
            return false;
        }
    }
    return true;
...
bool isMaximalClique(vector<int>& seq, int V, vector<bool>& vis) {
    for (int v = 1; v <= V; v++) {
        if (!vis[v] && isConnected(seq, seq.size(), v)) { // all vertices
            return false;
        }
    }
    return true;
